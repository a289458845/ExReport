...
- (NSUInteger)daysPerYearUsingReferenceDate:(NSDate *)date
{
    ...
    NSDateComponents *components = [self components:NSCalendarUnitYear fromDate:today];
    NSUInteger months = [self rangeOfUnit:NSCalendarUnitMonth
                                       inUnit:NSCalendarUnitYear
                                      forDate:today].length;
    for (int i = 1; i <= months; i++) {
        components.month = i;
        NSDate *month = [self dateFromComponents:components];
        days += [self rangeOfUnit:NSCalendarUnitDay
...
{
    return [self weeksPerYearUsingReferenceDate:[NSDate date]];
}
