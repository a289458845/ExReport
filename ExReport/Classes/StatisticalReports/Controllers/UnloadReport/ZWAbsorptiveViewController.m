...
@interface ZWAbsorptiveViewController ()
<UITableViewDelegate,
...
@implementation ZWAbsorptiveViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    if (self.controllerType == ZWAbsorptiveViewControllerMost) {
        self.OrderBy = 1;
        [self setupNavigationTitleWithTitle:@"消纳量最多消纳点"];
    } else if (self.controllerType == ZWAbsorptiveViewControllerless) {
        self.OrderBy = 2;
        [self setupNavigationTitleWithTitle:@"消纳量最少消纳点"];
    }
...
- (void)loadData{
    ...
    [[ExNetwork sharedManager] PostUrlString:URL_POST_GetUnLoadRank parameters:params success:^(id  _Nullable responseObject) {
        if ([responseObject[@"Code"] integerValue] == 0) {
            [ExProgressHUD hide];
