//
//  ZWCarOverspeedViewController.m
//  Muck
//
...
@interface ZWCarOverspeedViewController ()
<UITableViewDelegate,
...
@implementation ZWCarOverspeedViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.AlarmType = @[@"106"];
    
    if (self.controllerType == ZWCarOverspeedViewControllerMost) {
            self.OrderBy = 1;
        [self setupNavigationTitleWithTitle:@"超速时长最高车辆"];
    } else if (self.controllerType == ZWCarOverspeedViewControllerLess) {
             self.OrderBy = 2;
        [self setupNavigationTitleWithTitle:@"超速时长最低车辆"];
    }
    self.Type = 2;
    [self loadDate];
