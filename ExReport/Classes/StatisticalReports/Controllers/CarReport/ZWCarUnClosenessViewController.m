//
//  ZWCarUnClosenessViewController.m
//  Muck
//
...
@interface ZWCarUnClosenessViewController ()
<UITableViewDelegate,
...
@implementation ZWCarUnClosenessViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.AlarmType = @[@"100"];
    if (self.controllerType == ZWCarUnClosenessViewControllerMost) {
        self.OrderBy = 1;
        [self setupNavigationTitleWithTitle:@"未密闭时长最高车辆"];
        
    } else if (self.controllerType == ZWCarUnClosenessViewControllerLess) {
        self.OrderBy = 2;
        [self setupNavigationTitleWithTitle:@"未密闭时长最低车辆"];
    }
    self.Type = 2;
